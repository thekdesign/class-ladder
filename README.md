...
7 道題，從**收入、資產、安全網、認知環境到時間主導權**，算出你在台灣社會結構中的實質定位。
四個級距（生存防守 / 中產陣痛 / 翻身 / 頂層）的邊界與結果文案，都對照**主計總處、內政部不動產資訊平台、財政部**的真實統計校準 —— 不只是心理測驗。
...
| 路徑 | 作用 |
| --- | --- |
| `resources/js/data/questions.js` | 7 題題庫（含對照真實數據的選項註記） |
...
- `/result` 結果：級距定位 + 光譜標記 + 構面長條 + 對照真實數據（含出處）+ 分享
